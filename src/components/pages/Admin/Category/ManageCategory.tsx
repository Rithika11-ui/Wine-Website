...
import { useAdminCategories, CategoryForm } from '../../../../Hook/AdminCategories'
import { Categories } from './CategoriesData'

const EMPTY_FORM: CategoryForm = { name: '', description: '', status: 'Active' }
...
export default function ManageCategory() {
  const {
    ...
    search, setSearch,
    filtered,
    totalProducts, activeCount, totalCategories,
    handleSave,
    handleDelete,
    handleToggleStatus,
  } = useAdminCategories()
  ...
  const [editTarget, setEditTarget] = useState<Categories | null>(null)
  const [form,       setForm]       = useState<CategoryForm>(EMPTY_FORM)
  const [deleteId,   setDeleteId]   = useState<number | null>(null)

  const openAdd  = () => { setEditTarget(null); setForm(EMPTY_FORM); setModalOpen(true) }
  const openEdit = (cat: Categories) => {
    setEditTarget(cat)
  ...
  const onSave = async () => {
    ...
    await handleSave(form, editTarget)
    closeModal()
  }
    ...
    <div className="min-h-screen bg-[#F4F7FE]">
      <div className="max-w-7xl mx-auto px-8 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <StatCard label="Total Categories" value={totalCategories} icon={<Layers size={20} className="text-white" />}      color="bg-[#580C1F]"    />
            ...
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="e.g. Sparkling Wine" value={form.name}

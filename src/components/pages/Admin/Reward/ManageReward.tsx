...
const INITIAL_REWARDS: Reward[] = [
  { id: 1, code: 'WINE20',     description: 'Summer 20% off all reds',         discount: '20',  discountType: 'Percentage', minOrder: '50',  usageLimit: 200, usedCount: 143, status: 'Active',   expiryDate: '2024-08-31' },
  { id: 2, code: 'WELCOME10',  description: 'New member welcome discount',      discount: '10',  discountType: 'Percentage', minOrder: '0',   usageLimit: 500, usedCount: 312, status: 'Active',   expiryDate: '2024-12-31' },
  { id: 3, code: 'SAVE15',     description: '$15 off orders over $80',          discount: '15',  discountType: 'Fixed',      minOrder: '80',  usageLimit: 100, usedCount: 100, status: 'Expired',  expiryDate: '2024-05-01' },
  { id: 4, code: 'ROSE2024',   description: 'Rosé season special',              discount: '12',  discountType: 'Percentage', minOrder: '40',  usageLimit: 150, usedCount: 28,  status: 'Inactive', expiryDate: '2024-09-30' },
  { id: 5, code: 'VIP30',      description: 'VIP members exclusive 30% off',   discount: '30',  discountType: 'Percentage', minOrder: '100', usageLimit: 50,  usedCount: 19,  status: 'Active',   expiryDate: '2024-10-15' },
  { id: 6, code: 'FLAT20',     description: 'Flat $20 off sparkling wines',     discount: '20',  discountType: 'Fixed',      minOrder: '60',  usageLimit: 80,  usedCount: 55,  status: 'Active',   expiryDate: '2024-07-20' },
]

const EMPTY_FORM = {
  code: '', description: '', discount: '', discountType: 'Percentage' as DiscountType,
  minOrder: '0', usageLimit: 100, status: 'Active' as Status, expiryDate: ''
}
...
export default function ManageReward() {
  ...
  const openEdit = (r: Reward) => {
    setEditTarget(r)
    setForm({ code: r.code, description: r.description, discount: r.discount, discountType: r.discountType, minOrder: r.minOrder, usageLimit: r.usageLimit, status: r.status, expiryDate: r.expiryDate })
    setModalOpen(true)
  }
  ...
  const handleSave = () => {
    ...
    if (editTarget) {
      setRewards(prev => prev.map(r => r.id === editTarget.id ? { ...r, ...form } : r))
    } else {
      setRewards(prev => [...prev, { id: Date.now(), ...form, usedCount: 0 }])
    }
    closeModal()
  }

  const handleDelete = (id: number) => { setRewards(prev => prev.filter(r => r.id !== id)); setDeleteId(null) }

  const totalUsed = rewards.reduce((s, r) => s + r.usedCount, 0)
  ...
  return (
    <div className=" min-h-screen bg-[#F4F7FE]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Expired"       value={rewards.filter(r => r.status === 'Expired').length}
          icon={<Clock size={20} className="text-white" />} color="bg-red-400" />
      </div>
